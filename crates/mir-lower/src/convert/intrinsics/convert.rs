...
//! | `CvtRnBf16x2F32`      | `cvt.rn.bf16x2.f32 d, hi, lo;`        |
...
/// Convert `cvt.rn.bf16x2.f32`: pack two f32 values into bf16x2 (u32)
/// using round-to-nearest-even.
pub(crate) fn convert_cvt_rn_bf16x2_f32(
    ctx: &mut Context,
    rewriter: &mut DialectConversionRewriter,
    op: Ptr<Operation>,
    _operands_info: &OperandsInfo,
    ...
    let operands: Vec<_> = op.deref(ctx).operands().collect();
    if operands.len() < 2 {
        return pliron::input_err_noloc!("cvt_rn_bf16x2_f32 requires 2 operands");
    }
