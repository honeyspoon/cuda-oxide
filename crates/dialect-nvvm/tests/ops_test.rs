...
use dialect_nvvm::ops::{
    Barrier0Op, Dp2aHiS32Op, Dp2aHiU32Op, Dp2aLoS32U32Op, Dp2aLoU32S32Op,
    Dp4aS32U32Op, Dp4aU32S32Op, ElectSyncOp, FmaBf16x2Op, LdmatrixX2Op,
    MmaM8N8K4F64Op, MmaM16N8K8F32Tf32Op, MmaM16N8K16F32Bf16Op, MmaM16N8K16F32F16Op,
    MmaM16N8K32S32S8Op, MovmatrixTransB16Op, NvvmAtomAddBf16x2Op,
    NvvmAtomAddF16x2Op, ReadPtxSregDynamicSmemSizeOp, ReadPtxSregGridIdOp,
    ReadPtxSregLaneIdOp, ReadPtxSregLanemaskEqOp, ReadPtxSregLanemaskGeOp,
    ReadPtxSregLanemaskGtOp, ReadPtxSregLanemaskLeOp, ReadPtxSregLanemaskLtOp,
    ReadPtxSregNsmIdOp, ReadPtxSregNwarpIdOp, ReadPtxSregSmIdOp,
    ReadPtxSregTidXOp, ReadPtxSregTotalSmemSizeOp, ReadPtxSregWarpIdOp,
    ReduxSyncAddOp, ReduxSyncAndOp, ReduxSyncMaxOp, ReduxSyncMinOp,
    ReduxSyncOrOp, ReduxSyncUmaxOp, ReduxSyncUminOp, ReduxSyncXorOp,
...
#[test]
fn test_dp_extended_ops_verify_three_i32_operands_and_one_i32_result() {
    let mut ctx = Context::new();
    dialect_nvvm::register(&mut ctx);
    ...
    // All six extended dot-product ops share the same shape:
    // 3 operands (all 32-bit integer), 1 result (32-bit integer).
    macro_rules! check_dp_variant {
        ($op:ty) => {{
            // Valid: 3 i32 operands, 1 i32 result.
            let valid = Operation::new(
                &mut ctx,
                <$op>::get_concrete_op_info(),
                vec![i32_ty.into()],
            ...
            verify_op(&<$op>::new(valid), &ctx).expect(concat!(
                stringify!($op),
                " must accept 3 i32 operands and 1 i32 result",
            ));
            ...
            verify_op(&<$op>::new(bad_operand), &ctx)
                .expect_err(concat!(stringify!($op), " must reject i64 operand"));
            ...
            verify_op(&<$op>::new(bad_result), &ctx)
                .expect_err(concat!(stringify!($op), " must reject f32 result"));
            ...
            verify_op(&<$op>::new(bad_arity), &ctx)
                .expect_err(concat!(stringify!($op), " must reject wrong operand count"));
            ...
            verify_op(&<$op>::new(bad_f32_operand), &ctx)
                .expect_err(concat!(stringify!($op), " must reject f32 operand"));
        }};
    }

    check_dp_variant!(Dp2aHiS32Op);
    check_dp_variant!(Dp2aHiU32Op);
    check_dp_variant!(Dp4aS32U32Op);
    check_dp_variant!(Dp4aU32S32Op);
    check_dp_variant!(Dp2aLoS32U32Op);
    check_dp_variant!(Dp2aLoU32S32Op);
}
