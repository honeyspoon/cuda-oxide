...
pub fn emit_fma_relu_bf16x2(
    ...
    if args.len() != 3 {
        return input_err!(
            loc.clone(),
            TranslationErr::unsupported(format!(
                "fma_relu_bf16x2 expects 3 arguments (a: u32, b: u32, c: u32), got {}",
                args.len()
            ))
        );
    }
    ...
    fma_relu_op.deref_mut(ctx).set_loc(loc.clone());

    if let Some(prev) = last_op {
        fma_relu_op.insert_after(ctx, prev);
    } else {
        fma_relu_op.insert_at_front(block_ptr, ctx);
    }

    let result = fma_relu_op.deref(ctx).get_result(0);
    emit_store_result_and_goto(
        ctx,
        destination,
        result,
...
// ---------------------------------------------------------------------------
// Public emit functions for each new op
// ---------------------------------------------------------------------------
