...
use dialect_nvvm::ops::{
    CvtF16x2F32Op, CvtRnBf16x2F32Op, CvtRnReluBf16x2F32Op, CvtRnReluF16x2F32Op, CvtRzBf16x2F32Op,
    CvtRzF16x2F32Op,
};
...
/// Emit cvt_rn_bf16x2_f32: convert two f32 values to packed bf16x2 (u32)
/// using round-to-nearest-even.
pub fn emit_cvt_rn_bf16x2_f32(
    ctx: &mut Context,
    body: &mir::Body,
    args: &[mir::Operand],
    destination: &mir::Place,
    ...
    if args.len() != 2 {
        return input_err!(
            loc.clone(),
            TranslationErr::unsupported(format!(
                "cvt_rn_bf16x2_f32 expects 2 arguments, got {}",
                args.len()
            ))
        );
    }
    ...
    emit_store_result_and_goto(
        ...
        block_map,
        loc,
        "cvt_rn_bf16x2_f32 call without target block",
    )
}
